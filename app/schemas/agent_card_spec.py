...
class AgentCapabilities(BaseModel):
    ...
    streaming: Optional[bool] = Field(
        None, description="If the Agent supports Server-Sent Events (SSE)"
    )
    ...
    stateTransitionHistory: Optional[bool] = Field(
        None, description="If the Agent exposes task state change history"
    )
    supportsAuthenticatedExtendedCard: Optional[bool] = Field(
        None,
        description="If the Agent supports authenticated extended card retrieval"
    )
...
class SecurityScheme(BaseModel):
    ...
    credentials: Optional[str] = Field(
        None, description="Credentials for the client to use for private Cards"
    )
...
class AgentSkill(BaseModel):
    ...
    tags: List[str] = Field(
        ..., description="Tags describing the skill's capability category"
    )
    examples: Optional[List[str]] = Field(
        None, description="Example scenarios or prompts the skill can execute"
    )
    inputModes: Optional[List[str]] = Field(
        None, description="Input MIME types supported by the skill"
    )
    outputModes: Optional[List[str]] = Field(
        None, description="Output MIME types supported by the skill"
    )
...
class AgentCardSpec(BaseModel):
    ...
    provider: Optional[AgentProvider] = Field(
        None, description="Agent Provider Object - Service provider information"
    )
    ...
    skills: List[AgentSkill] = Field(
        ..., description="Agent Skill Objects - Collection of capability units"
    )
    ...
    documentationUrl: Optional[HttpUrl] = Field(
        None, description="URL for the Agent's documentation"
    )
