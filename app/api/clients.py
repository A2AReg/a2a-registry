...
async def get_client(
    client_id: str, current_client=Depends(require_admin), db: Session = Depends(get_db)
):
    """Get a client by ID."""
    client_service = ClientService(db)
...
@router.post(
    ...
    response_model=ClientEntitlementResponse,
...
@router.delete(
    "/{client_id}/entitlements/{entitlement_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_client_entitlement(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Entitlement not found"
        )
