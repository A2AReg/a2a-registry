...
class PeeringService:
    ...
    def get_peer_by_url(self, base_url: str) -> Optional[PeerRegistry]:
        ...
        return self.db.query(PeerRegistry).filter(PeerRegistry.base_url == base_url).first()
    
    def update_peer(self, peer_id: str, peer_data: PeerRegistryUpdate) -> Optional[PeerRegistry]:
    ...
    def get_sync_history(self, peer_id: Optional[str] = None, limit: int = 50) -> List[PeerSync]:
    ...
    def get_peers_due_for_sync(self) -> List[PeerRegistry]:
        ...
        return self.db.query(PeerRegistry).filter(
            and_(
